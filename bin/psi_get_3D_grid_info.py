...
def main():
    ...
    vec = dr_vec
    print(format_string % ('dr',np.min(vec),np.max(vec),np.max(vec)/np.min(vec),np.mean(vec),np.std(vec)))
    vec = dt_vec
    ...
    print()
    vec = vol
    print(format_string % ('dV ('+unit_str+'^3)',np.min(vec),np.max(vec),np.max(vec)/np.min(vec),np.mean(vec),np.std(vec)))
    ...
    vec = ddr_vec
    print(format_string % ('|ddr|',np.min(vec),np.max(vec),' ',np.mean(vec),np.std(vec)))
    vec = ddt_vec
    ...
    print(format_string % ('|ddr|/dr',np.min(vec),np.max(vec),' ',np.mean(vec),np.std(vec)))
